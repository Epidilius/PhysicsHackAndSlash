...
namespace GameDev2D
{
    ...
    void Button::HandleEvent(Event* aEvent)
    {
        if(IsEnabled() == true)
        {
            switch (aEvent->GetEventCode())
            {
                case TOUCH_EVENT:
                {
                    SetState(HandleTouchInput((TouchEvent*)aEvent));
                }
                break;
                
                case MOUSE_CLICK_EVENT:
                {
                    SetState(HandleMouseClickInput((MouseClickEvent*)aEvent));
                }
                break;
    ...
    ButtonState Button::HandleTouchInput(TouchEvent* aTouchEvent)
    {
        switch (aTouchEvent->GetTouchType())
        {
            case TouchBegan:
            {
                if(ContainsPoint(aTouchEvent->GetTouchLocation()) == true)
                {
                    ...
                    {
                        return GetState() == ButtonStateNormal ? ButtonStateSelected : ButtonStateNormal;
                    }
                }
            }
            break;
    ...
    ButtonState Button::HandleMouseClickInput(MouseClickEvent* aMouseClickEvent)
    {
